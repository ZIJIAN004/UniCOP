...
**OpenRLHF 0.10.2** 引入了**层级式 dot notation CLI**（`--algo.advantage.estimator` 而非 `--advantage_estimator`）。本目录所有脚本已适配此格式。
...
|---|---|---|
| GRPO 算法 | ✅ | ✅ `--algo.advantage.estimator group_norm` |
| DAPO clip-higher（非对称 ratio） | ✅ 手写 | ✅ `--actor.eps_clip_low_high 0.2 0.28` |
| POMO PRM（step-level） | ✅ 走 `grpo_prm_trainer.py` 改装 | ⚠️ 聚合为 scalar（远程 reward 接口限制） |
| Terminal reward | ✅ | ✅ |
| NoRepeatNgram（n=6） | ✅ 走 `trl vllm-serve` | ⚠️ 需 entry point 注册 + 环境变量 fallback |
| LoRA rank=64 | ✅ | ✅ `--ds.lora.rank 64` |
| ZeRO-3 / FSDP | ✅ ZeRO-3 | ✅ `--ds.zero_stage 3` |
| vLLM rollout | ✅ 独立 1 卡 | ✅ Ray 托管 |
| 调度自动化 | ⚠️ 手写 200 行 bash | ✅ Ray 原生 |

### 已知局限：step-level PRM

OpenRLHF 的 `--reward.remote_url` 接口返回**completion 级 scalar reward**，无法像父目录 `grpo_prm_trainer.py` 那样在 token 位置提供 per-step advantage。
...
### 已知局限：NoRepeatNgram 注册

OpenRLHF 0.10.2 不透传 vLLM 的 `--logits-processors` CLI 参数，也不存在 `VLLM_LOGITS_PROCESSORS` 环境变量。

当前方案：

1. `ngram_processor.py` 使用环境变量 `NO_REPEAT_NGRAM_SIZE` 作为 fallback（默认 6）
2. 通过 pyproject.toml entry point 注册到 vLLM（需 `pip install -e .`）
3. 若 entry point 方式不生效，需 fork OpenRLHF 的 `vllm_engine.py`

## 接口规范（OpenRLHF 0.10.2）

### Remote Reward Server (`/get_reward`)
...
```json
{
    ...
    "prompts": ["仅 prompt 文本"],
    "labels": ["instance_id 字符串 (来自 --data.label_key)"]
}
```
...
```json
{
    "rewards": [float],
    "scores": [float, 0-1 范围],
    "extra_logs": {"raw_rewards": [float]}
}
```

### Colocate Reward Function
...
├── install.sh                 conda env 创建 + 完整安装（含 flash-attn 源码编译）
├── scripts/
...
│   └── train_grpo_tsp20_7b.sh    启动脚本（4 卡 3090 · 7B · LoRA · ZeRO-3 + offload）
...
# 5. 另一个终端启动训练
bash configs/train_grpo_tsp10_1.5b.sh   # 1.5B TSP10 (8卡)
# 或
bash configs/train_grpo_tsp20_7b.sh     # 7B TSP20 (4卡)
```

## 首次运行前的服务器验证清单

> 修复后的代码基于社区文档和源码推断，部分参数名（LoRA、attn_implementation 等）需在服务器实际验证。

```bash
# 验证 CLI 参数是否被正确识别
python -m openrlhf.cli.train_ppo_ray --help 2>&1 | head -100

# 验证 LoRA 参数名
python -m openrlhf.cli.train_ppo_ray --help 2>&1 | grep -i lora

# 验证 NoRepeatNgram entry point 是否注册成功
python -c "from importlib.metadata import entry_points; eps = entry_points(); print([ep for ep in eps.get('vllm.logits_processors', [])])"
```
...
- [x] Reward server + reward_fn（适配 0.10.2 接口规范）
- [x] NoRepeatNgram processor（环境变量 fallback）
- [x] 启动脚本（适配 0.10.2 层级式 CLI）
- [ ] 服务器验证 CLI 参数 + 首次训练
