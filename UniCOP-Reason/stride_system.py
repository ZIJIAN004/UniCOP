"""stride>1 思维链 system 覆盖 (RL 训练数据集共用)。

prob.build_prompt 给的是 stride=1 的 _SYSTEM, 但 SFT 训练数据 (build_think_chains --stride N)
用的是 problems_prompt.get_system_prompt(pt, N) 的批量版 system。RL 训练若不替换, prompt 与
SFT 模型期望不一致 (train/eval 失配)。这里用与 build_think_chains 同一函数 (单一真源) 覆盖 system,
保证逐字一致; user (实例描述) 不变。
"""
import os
import sys
