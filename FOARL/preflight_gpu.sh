...
#   从 UniCOP-Reason-Mask/preflight_gpu.sh 原样复制, 保持 FOARL 自包含。
#
...
#   SUBMIT_SCRIPT  — 要重投的 submit 绝对路径 (如 "$(pwd)/submit_grpo_foarl_cvrp.sh")
...
preflight_gpu_or_resubmit() {
    ...
    # (MODEL/LR/... 都在本 job 环境里), 再叠加 bookkeeping。
