...
# POMO ckpt 守卫: POMOPRM 实际 glob "*POMO_CVRP_n20" 取最新, 加载其中的 MODEL_FINAL.pt
POMO_CVRP_DIR=$(ls -d "$POMO_CKPT_DIR"/*POMO_CVRP_n20 2>/dev/null | tail -1)
if [ -z "$POMO_CVRP_DIR" ] || [ ! -f "$POMO_CVRP_DIR/MODEL_FINAL.pt" ]; then
  echo "❌ POMO ckpt 问题: 需要 $POMO_CKPT_DIR/*POMO_CVRP_n20/MODEL_FINAL.pt (wave 剪枝必需)"
  echo "   匹配到目录: ${POMO_CVRP_DIR:-<无>}"
  [ -n "$POMO_CVRP_DIR" ] && { echo "   目录内容:"; ls -la "$POMO_CVRP_DIR"; \
    echo "   若只有 MODEL_BEST.pt, 做软链: ln -s MODEL_BEST.pt $POMO_CVRP_DIR/MODEL_FINAL.pt"; }
  exit 1
fi
