...
echo "  Reward:    v5 (hardgate distance + cov/cons 加权)"
echo "             A_feas = w_p×parse + w_cov×cov + w_cons×cons(gate) + w_f×format"
echo "                      权重 0.5/2.5/2.0/0.5; mask 下 cov+parse+format 同质,"
echo "                      A_feas 信号主要靠 cons (w_cons=2.0, ~36%)"
echo "             A_outcome = z(-raw_distance) on strict fully_feasible 子集 >=2"
echo "                         mask 下子集大 (~30-50%), distance 优化常态启用"
...
echo "  LR:        2e-5 (v5 默认), warmup 5 step"
