...
#                   训练阶段腾空的卡给 vLLM: TP=2 (GPU 0,1, matrix 实测推荐), 比默认 TP=1 快
...
#     卡自动释放回本 shell 的 SLURM 配额, eval 直接拿来用 (默认 TP=2)。eval 阶段没用满
#     7 卡——可接受 (换取流程单一、无跨 job 依赖、无 QOS 二次排队)。
#
...
EVAL_GPU="${EVAL_GPU:-0,1}"              # 训练完腾空的卡给 vLLM; TP=2 用 GPU 0,1
EVAL_TP="${EVAL_TP:-2}"                  # tensor parallel; 2 = matrix 实测推荐值 (KV 翻倍减抢占)。
                                         # 想用更多卡: 确认 Qwen3-4B num_key_value_heads 可整除后,
                                         # 如 kv=8 → EVAL_TP=4 EVAL_GPU=0,1,2,3 sbatch (TP 必须整除 kv_heads)。
