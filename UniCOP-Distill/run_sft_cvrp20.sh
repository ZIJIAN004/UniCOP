#!/bin/bash
# CVRP20 直接 SFT（跳过数据收集），使用已有的 chains 数据
#
# 前提: data/chains_self_cvrp20.jsonl 已存在
#
# 使用方法：
#   bash run_sft_cvrp20.sh
#   nohup bash run_sft_cvrp20.sh > sft_cvrp20.log 2>&1 &

set -euo pipefail

_SELF_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_FILE="$_SELF_DIR/sft_cvrp20_$(date '+%Y%m%d_%H%M%S').log"
...
trap 'notify "CVRP20 SFT 失败: line $LINENO"' ERR
...
notify "CVRP20 SFT 全部完成" "模型: $OUTPUT_DIR, 数据: $DATA_COUNT 条, 评估结果在 eval_results/"
